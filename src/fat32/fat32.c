...
static void write_fat_to_disk(fat32_fs_t *fs) {
  ...
  // Write both FAT copies to keep them in sync.
  pi_sd_write(fs->fat, fs->fat_begin_lba, boot_sector.nsec_per_fat);
  pi_sd_write(fs->fat, fs->fat_begin_lba + boot_sector.nsec_per_fat, boot_sector.nsec_per_fat);
}
...
static void write_cluster_chain(fat32_fs_t *fs, uint32_t start_cluster, uint8_t *data, uint32_t nbytes) {
  ...
  // If nothing to write, don't use any clusters.
  if (nbytes == 0) {
    // Free entire existing chain if there is one.
  ...
  // Walk existing chain, writing data to each cluster.
  ...
  // If we still have data to write, allocate new clusters.
  while (written < nbytes) {
    uint32_t new_cluster = find_free_cluster(fs, 3);
    // Link previous cluster to new one.
    if (prev)
      fs->fat[prev] = new_cluster;
    fs->fat[new_cluster] = 0x0FFFFFFF; // LAST_CLUSTER
  ...
  if (prev && fat32_fat_entry_type(fs->fat[prev]) != LAST_CLUSTER &&
    ...
    fs->fat[prev] = 0x0FFFFFFF; // Ensure last cluster is marked.
  }
...
pi_dirent_t *fat32_create(fat32_fs_t *fs, pi_dirent_t *directory, char *filename, int is_dir) {
  ...
  // Find a free entry (first byte 0x00 or 0xE5).
  int free_idx = -1;
  for (uint32_t i = 0; i < n; i++) {
    if (dirents[i].filename[0] == 0x00 || dirents[i].filename[0] == 0xE5) {
      free_idx = i;
      break;
    }
  }
...
int fat32_truncate(fat32_fs_t *fs, pi_dirent_t *directory, char *filename, unsigned length) {
  ...
  if (length == 0) {
    ...
    // File was empty, allocate a cluster.
    uint32_t new_clust = find_free_cluster(fs, 3);
    fs->fat[new_clust] = 0x0FFFFFFF;
...
int fat32_write(fat32_fs_t *fs, pi_dirent_t *directory, char *filename, pi_file_t *file) {
  ...
  // If file currently has no cluster and new data is non-empty, allocate one.
  if (clust < 2 && file->n_data > 0) {
    clust = find_free_cluster(fs, 3);
    fs->fat[clust] = 0x0FFFFFFF; // LAST_CLUSTER
  ...
  // Write file data to cluster chain.
  if (file->n_data > 0)
