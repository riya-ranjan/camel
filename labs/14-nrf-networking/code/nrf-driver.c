...
nrf_t *nrf_init(nrf_conf_t c, uint32_t rxaddr, unsigned acked_p) {
    ...
    if(!acked_p) {
        ...
        nrf_put8_chk(n, NRF_EN_RXADDR, 0b00000010);
        nrf_put8_chk(n, NRF_SETUP_RETR, 0);   // retransmission disabled
        ...
        uint32_t converted_retran_delay = nrf_default_retran_delay / 250 - 1;
        nrf_put8_chk(n, NRF_SETUP_RETR, converted_retran_delay << 4 | nrf_default_retran_attempts);
    ...
    dev_barrier();
    gpio_set_output(c.ce_pin);
    dev_barrier();
    nrf_put8_chk(n, NRF_CONFIG, 1);         // switch to standby mode
    dev_barrier();
    gpio_set_off(c.ce_pin);
    dev_barrier();
    delay_us(150);
...
static void nrf_rx_mode(nrf_t *n) {
    ...
    dev_barrier();
    // delay_ms(5);
    nrf_opt_assert(n, nrf_is_standbyI(n));
    nrf_put8_chk(n, NRF_CONFIG, rx_config);
    dev_barrier();
...
int nrf_get_pkts(nrf_t *n) {
    ...
    while (!nrf_rx_fifo_empty(n)) {
        if (nrf_rx_get_pipeid(n) != 1) {
            panic("got packet for wrong pipe\n");
        }
        uint8_t buf[n->config.nbytes];
