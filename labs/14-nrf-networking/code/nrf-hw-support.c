...
//
// NOTE: if you want to use software SPI you'll have to 
// change this interface so you can specify the pins to use.
spi_t nrf_spi_init(unsigned ce, unsigned spi_chip) {
...
// read the value of a single NRF <reg>.
//
// NOTE: all register names and values are 8-bits.
//
...
// set NRF <reg> = <v>
//
// could rewrite this to use <nrf_getn> to reduce lines of code.
// we did it like this so it's a simpler bit of code to inspect at the 
// very beginning. this to use putn
uint8_t nrf_put8(nrf_t *n, uint8_t reg, uint8_t v) {
    uint8_t rx[3], tx[3];

    // note: tell the hardware that this is a write, by or-ing Din
    // NRF_WR_REG [from datasheet]
    tx[0] = NRF_WR_REG | reg;
    tx[1] = v;
...
// set as expected.
//
// passes in the calling location information <loc> so we can emit
// useful errors.
uint8_t 
nrf_put8_chk_helper(src_loc_t loc, nrf_t *nic, uint8_t reg, uint8_t v) {
