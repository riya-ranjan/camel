...
static inline void cycle_cnt_write(uint32_t in) {
    asm volatile("MCR p15, 0, %0, c15, c12, 1" :: "r"(in) : "memory");
}

static inline void uart_delay_ticks(uint32_t ticks) {
    // cycle_cnt_write(0);
    while (cycle_cnt_read() < ticks);
}
